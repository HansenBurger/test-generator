...
PROCESS_POINT_SUMMARY_PROMPT = """你是一个银行业务测试专家，需要将“流程测试点”归纳为简短可读的流程概要。
...
- point_id
- summary: 简短流程概括（8~20字，优先表达业务行为+结果）

要求：
1. 概括主流程，不罗列系统名称和细节步骤
2. 尽量使用“单笔/批量/成功/失败”等结果性词汇（以测试点语义为准）
3. 仅输出严格 JSON，不要解释或代码块

示例：
输入：[{"point_id":"P1","text":"流程作业发起保费收取申请，资产调度平台接收申请，系统检查通过，生成保费收取结果成功"}]
输出：[{"point_id":"P1","summary":"单笔保费收取成功"}]
"""
